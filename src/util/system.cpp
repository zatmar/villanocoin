...
const char * const BITCOIN_CONF_FILENAME = "villanocoin.conf";
...
#else
    const char* pszModule = "villanocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Villanocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Villanocoin";
#else
    // Unix
    return pathRet / ".villanocoin";
#endif
#endif
}
